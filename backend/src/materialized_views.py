...
def _build_refresh_sql_sqlite(window_days: int) -> str:
    txn_where = ""
    registration_where = ""
    if window_days > 0:
...
def _build_refresh_sql_postgres(window_days: int) -> str:
    txn_where = ""
    registration_where = ""
    if window_days > 0:
        txn_where = f"WHERE fecha_hora >= (NOW() - INTERVAL '{window_days} days')"
        registration_where = f"WHERE fecha_registro >= (CURRENT_DATE - INTERVAL '{window_days} days')"
...
INSERT INTO mv_refresh_state (id, last_refresh) VALUES (1, NULL)
...
SELECT
    day,
    SUM(CASE WHEN estado = 'Exitosa' THEN monto_total ELSE 0 END) AS total_sales,
    SUM(CASE WHEN estado = 'Exitosa' THEN ganancia_neta ELSE 0 END) AS total_profit,
    SUM(CASE WHEN estado = 'Exitosa' THEN 1 ELSE 0 END) AS txn_count
FROM temp_base_txn
GROUP BY day;
...
DELETE FROM mv_customers_registration_daily
WHERE day IN (
    SELECT DISTINCT fecha_registro::date
    FROM clientes
    {registration_where}
);
INSERT INTO mv_customers_registration_daily (day, new_customers)
SELECT
    fecha_registro::date AS day,
    COUNT(*) AS new_customers
FROM clientes
{registration_where}
...
UPDATE mv_refresh_state SET last_refresh = NOW() WHERE id = 1;
"""
...
def refresh_materialized_views(full_refresh: bool = False) -> bool:
    try:
        ...
        if engine == "postgres":
            sql = _build_refresh_sql_postgres(window_days)
        else:
            sql = _build_refresh_sql_sqlite(window_days)
        return execute_write_query(sql)
